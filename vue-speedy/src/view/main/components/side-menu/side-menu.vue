                    ...
                    <Tooltip v-else
                        ...
                        <a @click="handleSelect(getNameOrHref(item, true))" class="drop-menu-a"
                           :style="{textAlign: 'center'}">
                            <common-icon v-if="commonIconType(item)"
                                         :size="rootIconSize"
                                         :color="textColor"
                                         :type="commonIconType(item)"/>
                        </a>
                    </Tooltip>
                </template>
            </div>
...
import {getRouteByName, routesToParallel} from "../../../../libs/util";
...
export default {
	...
	methods: {
		handleSelect (name) {
		    let route = getRouteByName(this.menuList, name)
			this.$emit('on-select', route)
		},
		getOpenedNamesByActiveName (name) {
			return this.$route.matched.map(item => item.name).filter(item => item !== name)
		},
        commonIconType(item){
            return item.icon || (item.children && item.children[0] && item.children[0].icon)
        }
	},
	computed: {
		textColor () {
